...
const DEVICE_ID_KEY = "referidos:rn:device-id";
...
async function normalizeFunctionError(error: any) {
  let code: string | number = "function_invoke_failed";
  let status = null;
  ...
  if (response && typeof response === "object") {
    ...
    try {
      const payload =
        typeof response.clone === "function" && typeof response.clone().json === "function"
...
async function invokeSessionFunction(name: string, body: Record<string, any> = {}) {
  const {
    data: { session } = {},
...
function formatLastActive(isoDate: string) {
...
export async function registerCurrentSessionDevice() {
  ...
  return invokeSessionFunction("session-register", {
    device_id: deviceId,
...
export async function listCurrentUserSessions() {
  const response: any = await invokeSessionFunction("session-list");
...
export async function revokeSessionById(sessionId: string) {
  return invokeSessionFunction("session-revoke", { session_id: sessionId });
}

export async function revokeAllSessions() {
  return invokeSessionFunction("session-revoke-all");
}
