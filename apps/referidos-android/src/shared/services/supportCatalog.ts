...
function normalizeArray(values: any, fallback: string[] = []) {
...
function normalizeAudienceRoles(values: any, fallback = ["cliente", "negocio"]) {
  const normalized = Array.from(
    new Set(
...
function normalizeThreadStatus(value: any) {
  const normalized = asString(value).toLowerCase().replace(/[\s-]+/g, "_");
  return THREAD_STATUS_ALIASES.get(normalized) || normalized;
}

function normalizeCategoryCode(value: any) {
...
export function normalizeSupportAppKey(value: any, fallback = "undetermined") {
  const normalized = asString(value).toLowerCase();
  if (!normalized) return fallback;
  return APP_ALIASES.get(normalized) || fallback;
}

export function normalizeSupportEnvKey(value: any, fallback = "dev") {
  const normalized = asString(value).toLowerCase();
  if (!normalized) return fallback;
...
export async function loadSupportCatalogFromCache({ publishedOnly = true } = {}) {
  ...
  const normalizedMacros = (macros || []).map((macro: any) => ({
    ...macro,
...
export function filterSupportMacrosForThread({
  ...
  const appKey = normalizeSupportAppKey(thread.app_channel || "", "referidos_app");
    ...
    (categories || [])
      .map(
        (category): [string, string] => [
          asString(category.id),
          normalizeCategoryCode(category.code || category.id),
        ],
      )
      .filter(([id, code]) => id && code),
  );

  const sortMacros = (list: any[]) =>
    list.sort((a, b) => {
  ...
  const baseFiltered = (macros || []).filter((macro) => {
    ...
    const categoryCode = normalizeCategoryCode(
      macro.category_code || macro.category || categoryCodeById.get(asString(macro.category_id)) || "",
    );
    if (categoryCode) {
