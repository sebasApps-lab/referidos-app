...
import { markSupportWhatsAppNameChanged } from "../_shared/supportAutoAssign.ts";
...
serve(async (req) => {
  ...
  const targetLabel = safeTrim(body.target_label, 120);
  if (!threadPublicId) {
  ...
  const result = await markSupportWhatsAppNameChanged({
    threadId: thread.id,
    actorId: usuario.id,
    actorRole: usuario.role === "admin" ? "admin" : "soporte",
    targetLabel,
  });
  if (!result.ok) {
    return jsonResponse({ ok: false, error: result.error || "whatsapp_name_update_failed" }, 500, cors);
  }
  ...
  return jsonResponse(
    {
      ...
      whatsapp_name_changed_at: result.whatsapp_name_changed_at,
    },
    200,
    cors,
  );
