...
serve(async (req) => {
  ...
  const qrCode = body.code;

  if (!qrId && !qrCode) {
    return jsonResponse({ ok: false, error: "missing_qr_identifier" }, 400, cors);
  }

  const now = new Date().toISOString();
  let query = supabaseAdmin.from("qr_validos").update({
    status: "expirado",
    expires_at: now,
  });
  ...
  if (qrId) {
    ...
    query = query.eq("code", qrCode);
  }

  const { data, error } = await query
    .select("id, code, status, expires_at, promo_id, negocio_id, cliente_id")
    .single();
  ...
  return jsonResponse({ ok: true, qr: data }, 200, cors);
});
