...
import {
  ArchivedBusinessDownloadButton,
  ArchivedJoinWaitlistButton,
} from "./styles/buttonFullArchive";

const FLOW_TARGET_ID = "waitlist-flow";

class ButtonFallbackBoundary extends React.Component {
  constructor(props) {
    super(props);
...
export default function WaitlistPage() {
  ...
  const shouldAnimateEntry = entryAnimTick > 0;
  ...
  return (
      ...
      <main className="relative z-10">
          ...
          <div className="hero-content">
            ...
            >
              <div
                className={`${shouldAnimateEntry ? "fade-edge-left " : ""}flex items-center gap-3 text-lg font-semibold tracking-tight text-[var(--ink)]`}
              >
              ...
              <div
                ...
                className={`${shouldAnimateEntry ? "fade-edge-right " : ""}flex items-center rounded-full bg-white/90 px-0.5 py-0.5 shadow-lg backdrop-blur`}
              >
                <button
                  type="button"
              ...
              <div
                className={`${shouldAnimateEntry ? "fade-down " : ""}hidden absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-10 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 md:flex`}
              >
              ...
              <div className="mini-nav-purple-slice pointer-events-none hidden absolute inset-0 md:block">
                <div
                  className={`${shouldAnimateEntry ? "fade-down " : ""}absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-10 text-xs font-semibold uppercase tracking-[0.2em] text-white/90`}
                >
                  {visibleMiniNavItems.map((item) => (
            ...
            <div className="hero-grid">
              <div className="hero-left-col space-y-6">
                <div
                  key={`hero-left-${modeAnimKey}`}
                  className={`${shouldAnimateEntry ? "fade-edge-left " : ""}space-y-6`}
                >
              ...
              <div className="hero-right hero-right-col flex w-full -translate-y-3 flex-col items-stretch md:items-end">
                <div
                  key={`hero-right-${modeAnimKey}`}
                  className={`${shouldAnimateEntry ? "fade-edge-right " : ""}flex w-full flex-col items-stretch md:items-end`}
                >
        ...
        >
          <div
            key={`section-two-${modeAnimKey}`}
            className={`${shouldAnimateEntry ? "fade-up " : ""}rounded-[36px] border border-slate-400/45 bg-white/55 p-6 shadow-xl backdrop-blur`}
          >
        ...
        <section id="mas-informacion" ref={sectionThreeRef} className="scroll-stage mx-auto w-full max-w-6xl px-6 pb-48 pt-52">
          <div
            key={`section-three-${modeAnimKey}`}
            className={`${shouldAnimateEntry ? "fade-up " : ""}rounded-[28px] border border-slate-400/45 bg-white/85 px-6 pb-6 pt-10 shadow-lg`}
          >
            {uiMode === "cliente" ? (
              <div>
