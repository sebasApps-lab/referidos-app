...
------------------------------------------------
with qr_data (promo_id, negocio_id, cliente_id, status, ts_offset_min) as (
  -- cliente USR_CLI_1
  values
    ('PRO_001', 'NEG_001', 'USR_CLI_1', 'canjeado', -25),
    ('PRO_003', 'NEG_002', 'USR_CLI_1', 'canjeado', -50),
    ('PRO_005', 'NEG_003', 'USR_CLI_1', 'expirado', -120),
    ('PRO_012', 'NEG_006', 'USR_CLI_1', 'expirado', -200),
    -- cliente USR_CLI_2
    ('PRO_002', 'NEG_002', 'USR_CLI_2', 'canjeado', -35),
    ('PRO_014', 'NEG_007', 'USR_CLI_2', 'canjeado', -45),
    ('PRO_009', 'NEG_005', 'USR_CLI_2', 'expirado', -180),
    ('PRO_018', 'NEG_009', 'USR_CLI_2', 'expirado', -240),
    -- cliente USR_CLI_3
    ('PRO_010', 'NEG_005', 'USR_CLI_3', 'canjeado', -22),
    ('PRO_012', 'NEG_006', 'USR_CLI_3', 'canjeado', -55),
    ('PRO_015', 'NEG_007', 'USR_CLI_3', 'canjeado', -40),
    ('PRO_016', 'NEG_008', 'USR_CLI_3', 'canjeado', -70),
    ('PRO_004', 'NEG_002', 'USR_CLI_3', 'expirado', -260),
    ('PRO_006', 'NEG_003', 'USR_CLI_3', 'expirado', -190),
    -- cliente USR_CLI_4
    ('PRO_001', 'NEG_001', 'USR_CLI_4', 'canjeado', -32),
    ('PRO_017', 'NEG_008', 'USR_CLI_4', 'canjeado', -28),
    ('PRO_011', 'NEG_005', 'USR_CLI_4', 'expirado', -150),
    ('PRO_018', 'NEG_009', 'USR_CLI_4', 'expirado', -210)
)
, computed as (
  select
    promo_id,
    ...
    status,
    ts_offset_min,
    floor(extract(epoch from now() + (ts_offset_min || ' minutes')::interval))::bigint as ts_epoch,
    gen_random_uuid() as qr_uuid
  from qr_data
)
, signed as (
  select
    ...
    sign_qr(short_hash(negocio_id) || short_hash(promo_id) || short_hash(cliente_id) || ts_epoch::text, 8) as sig
  from computed
)
, ins as (
  ...
  select
    qr_uuid,
    format('qrv-%s-%s-%s-%s-%s', biz, prm, usr, ts_epoch::text, sig),
    promo_id,
    negocio_id,
    cliente_id,
    ts_epoch,
    to_timestamp(ts_epoch),
    to_timestamp(ts_epoch) + interval '30 minutes',
    case when status = 'canjeado' then to_timestamp(ts_epoch) + interval '5 minutes' else null end,
    status::qr_status,
    biz,
    prm,
    usr,
    sig,
    case when status = 'canjeado' then 'USR_NEG_1' else null end
  from signed
  returning id, cliente_id, ts_epoch, status
)
...
select
  ...
  id,
  cliente_id,
  to_timestamp(ts_epoch) + interval '6 minutes'
from ins
where status = 'canjeado';

insert into comentarios (id, promoId, clienteId, estrellas, texto, fechaCreacion) values
  ('COM_001', 'PRO_001', 'USR_CLI_1', 5, 'Excelente servicio y la promo funciono perfecto.', now() - interval '3 days'),
  ('COM_002', 'PRO_003', 'USR_CLI_1', 4, 'Buen cafe, volveria con amigos.', now() - interval '2 days'),
  ('COM_003', 'PRO_012', 'USR_CLI_1', 4, 'Buena atencion en la tienda, productos frescos.', now() - interval '1 day'),

  ('COM_004', 'PRO_002', 'USR_CLI_2', 5, 'La pizza salio rapidisimo y muy rica.', now() - interval '4 days'),
  ('COM_005', 'PRO_014', 'USR_CLI_2', 3, 'El descuento bien, pero falto stock.', now() - interval '2 days'),

  ('COM_006', 'PRO_016', 'USR_CLI_3', 5, 'Gran variedad y buenos precios.', now() - interval '5 days'),
  ('COM_007', 'PRO_010', 'USR_CLI_3', 4, 'Buen descuento en organicos.', now() - interval '3 days'),
  ('COM_008', 'PRO_006', 'USR_CLI_3', 4, 'El cafe estaba fresco y caliente.', now() - interval '1 day'),

  ('COM_009', 'PRO_001', 'USR_CLI_4', 4, 'La promo 2x1 cumplio lo prometido.', now() - interval '6 days'),
  ('COM_010', 'PRO_011', 'USR_CLI_4', 3, 'Me atendieron bien pero la promo ya estaba por terminar.', now() - interval '2 days');

insert into reportes (id, reporterId, reporterRole, targetId, targetType, texto, fechaCreacion) values
  ('REP_001', 'USR_CLI_1', 'cliente', 'PRO_012', 'promo', 'El descuento no aplicaba en todos los productos.', now() - interval '1 day'),
  ('REP_002', 'USR_CLI_2', 'cliente', 'NEG_003', 'negocio', 'Falto senalizacion para la promo.', now() - interval '2 days'),
  ('REP_003', 'USR_CLI_3', 'cliente', 'PRO_016', 'promo', 'No aceptaban el QR al inicio, luego funciono.', now() - interval '3 days'),
  ('REP_004', 'USR_CLI_4', 'cliente', 'NEG_001', 'negocio', 'Demora en validar el QR en caja.', now() - interval '4 days');
