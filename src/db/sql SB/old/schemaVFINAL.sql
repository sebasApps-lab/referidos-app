-- schema.sql
-- Versión FINAL — esquema listo para producción con integración a Supabase Auth.
-- Cambios mínimos necesarios según tus requerimientos:
--  - la tabla `usuarios` contiene id_auth (uuid) y emailConfirmado (boolean).
--  - NO se almacena password en la tabla (usar Supabase Auth).
--  - índices recomendados.
-- Ejecutar primero este archivo (crea las tablas desde cero).
...
create table usuarios (
  ...
  -- NOTA: no almacenamos passwords aquí; Supabase Auth gestiona contraseñas.
  telefono text,
  nombre text,
  apellido text,
  cedula text,
  ...
  -- usado por seed / frontend y por políticas (si requiere confirmación)
