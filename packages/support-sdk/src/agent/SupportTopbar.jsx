...
import { Bell, LogOut, Menu, RefreshCcw, ShieldCheck } from "lucide-react";
...
function resolveEnvironmentLabel() {
  ...
  if (envRaw === "production" || envRaw === "prod") return "Produccion";
  if (envRaw === "staging") return "Staging";
  return "Development";
}
...
export default function SupportTopbar({
  ...
  const initials = (usuario?.nombre || "Soporte")
    .split(" ")
    .slice(0, 2)
    .map((part) => part[0])
    .join("")
  ...
  return (
    <div className="support-topbar sticky top-0 z-40 border-b border-[#EEE7FA] bg-[#F6F2FB]/90 backdrop-blur">
